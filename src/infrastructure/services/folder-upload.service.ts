...
import { FolderNode, UploadSessionV2 } from "../../domain/models/upload-session-v2.model.js";
...
export class FolderUploadService {
  ...
  async uploadFolder(params: FolderUploadParams): Promise<UploadSessionV2> {
    log.info({ instructorId: params.instructorId, fileCount: params.files.length }, "Starting folder upload");
    ...
    if (totalSize > MAX_TOTAL_SIZE) {
      throw new Error(`Total upload size exceeds ${MAX_TOTAL_SIZE / (1024 * 1024 * 1024)}GB limit`);
    }
    ...
    for (const file of params.files) {
      const tempPath = this.generateTempStoragePath(params.instructorId, sessionId, file.originalname);
      ...
      log.debug({ file: file.originalname, path: tempPath }, "File uploaded to temp storage");
    }
    ...
    log.info({ sessionId: session.id, fileCount: params.files.length }, "Folder upload complete");
  ...
  async moveToStructuredStorage(
    sessionId: string,
    courseId: string,
    structure: { moduleId: string; lessonId: string; fileName: string; type: string }[]
  ): Promise<void> {
    ...
    for (const item of structure) {
      // Find temp file
      const tempPath = session.tempStoragePaths.find(path => path.includes(item.fileName));
      if (!tempPath) {
      ...
      log.debug({ from: tempPath, to: structuredPath }, "File moved to structured storage");
    }
    ...
    await this.sessionRepository.updateSession(sessionId, { status: 'complete' });
  ...
  private validateFiles(files: UploadedFile[]): void {
    for (const file of files) {
      ...
      if (file.size > MAX_FILE_SIZE) {
        throw new Error(`File ${file.originalname} exceeds ${MAX_FILE_SIZE / (1024 * 1024 * 1024)}GB limit`);
      }
    }
  }
   ...
   */
  private generateTempStoragePath(instructorId: string, sessionId: string, fileName: string): string {
  ...
  private generateStructuredPath(
    ...
    const extension = fileName.split('.').pop() || 'bin';
    const typeFolder = type === 'VIDEO' ? 'videos' : type === 'PDF' ? 'documents' : 'images';
   ...
   */
  private async uploadFileToS3(key: string, buffer: Buffer, _contentType: string): Promise<void> {
